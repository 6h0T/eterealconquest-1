...
// que verifica correctamente las variables de entorno y maneja los errores
...
export async function getStreamers(): Promise<Streamer[]> {
  try {
    ...
    if (!supabaseAdmin) {
      console.error("No se pudo conectar a Supabase. Credenciales no disponibles.")
      return []
    }
...
export async function ensureStreamersTable() {
  // This function is intentionally left blank as the table creation is handled by the SQL script.
  // This function is only present to satisfy the type checker in other modules.
}
